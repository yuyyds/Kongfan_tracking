...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        # # 计算应该经过的时间
        # expected_time = motion.current_idx * sim.cfg.dt  # 使用sim.cfg.dt而不是output_dt
        # actual_elapsed_time = time.time() - start_time
        
        # # 如果实际时间小于预期时间，等待
        # if actual_elapsed_time < expected_time:
        #     time.sleep(expected_time - actual_elapsed_time)

        # 使用的是 sim.render() 而不是 sim.step()，这意味着没有实际的物理仿真，只是渲染可视化
